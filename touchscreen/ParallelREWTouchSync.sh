...
do
	...
	then
		...
		then
			...
			cp /lustre03/project/rpp-markpb68/m3group/Haqqee/GitHub/SLURMstuff/behavtouchsync_REW.sl .
			cp /lustre03/project/rpp-markpb68/m3group/Haqqee/GitHub/SLURMstuff/behavtouchsync_REW.m .
			ID=$initials${session#*$initials}
			ID=${ID::5}
			date=202${session#*202}; date=${date::10}
			ID="$touch-$ID-$date" 
			sleep 2
			sed -i -e "s/TASKNAME/$ID/g" behavtouchsync_REW.sl
			sed -i -e "s/MYEMAIL/$email/g" behavtouchsync_REW.sl
			sbatch behavtouchsync_REW.sl
			sleep 2
		else
