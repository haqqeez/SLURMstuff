...
# if you want to test all videos in the directory, write "full"
...
downsampling+=(1 2) # spatial downsampling factor; usually 2-4  (we've always used 2)
...
gridness+=(1 3 5) # gridness; approx [n by n] number of patches; closer to 1 means basically rigid
...
upsampling+=(2 4 8) # spatial upsampling factor; usually 4
...
for session in $data_dirs; do
  ...
  for combination in $combinations; do
    ...
    sbatch --mail-user=$email \
          --mail-type=$mailtype \
          --job-name=$ID \
          --output=$sub_directory/${date}_${param1_placeholder,,}${i}_${param2_placeholder,,}${j}_${param3_placeholder,,}${k}_%j.out \
          --error="$sub_directory/errors/${date}_${param1_placeholder,,}${i}_${param2_placeholder,,}${j}_${param3_placeholder,,}${k}_%j.err" \
    ...
    sleep 0.8 # pause to be kind to the job scheduler on the server
  done
done
